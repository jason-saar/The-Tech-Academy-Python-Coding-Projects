...
# catch if the user's clicks on the windows upper-right 'X' to ensure they want close
def ask_quit(self):
...
def addToList(self):
    ...
    #normalize the data to keep it consisten in the database
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): #ensure that there is data present
        ...
        with conn:
            ...
            if count == 0 or count2 == 0: #if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request", "The following chnages ({}) and ({}) will be implemented for ({}). \n\nProceed with the update request?".format(var_phone,var_email,var_value))
                print(response)
                if response:
                    #conn = sqlite3.connect('db_phonebook.db')
                    with conn:
